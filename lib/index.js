...
module.exports = function parseMessage(response) {
  ...
  var mixedPartId = null;

  while (parts.length !== 0) {
    var part = parts.shift();
    if (part.parts) {
      parts = part.parts.concat(parts);
    }
    ...
    mixedPartId = part.mimeType && part.mimeType.indexOf('multipart/mixed') !== -1 ? part.partId : mixedPartId;
    ...
    if (isHtml && !isAttachment) {
        var decoded = urlB64Decode(part.body.data);
        if (!result.textHtml || mixedPartId == null || part.partId.indexOf(mixedPartId) !== 0) {
        ...
        var decoded = urlB64Decode(part.body.data);
        if (!result.textPlain || mixedPartId == null || part.partId.indexOf(mixedPartId) !== 0) {
            result.textPlain = urlB64Decode(part.body.data);
        } else {
            result.textPlain += '\n' + urlB64Decode(part.body.data);
        }
